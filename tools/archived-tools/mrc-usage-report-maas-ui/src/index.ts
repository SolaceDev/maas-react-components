...
program
	...
	.option("-m, --mfes <mfes>", "Comma-separated list of MFEs to analyze", "ep,intg,mc,saas")
...
async function main() {
	try {
		...
		// Parse options
		const mfes = options.mfes.split(",").filter(Boolean);
		...
		const fileScanner = new FileScanner(basePath, mfes, mrcSourceType, mrcGithubUrl, mrcGithubBranch);
		...
		const outputPath = path.join(outputDir, `mrc-maas-ui-usage-report.${outputFormat}`);
		...
		for (const format of outputFormats) {
			let currentOutputPath = path.join(outputDir, `mrc-maas-ui-usage-report.${format}`);
