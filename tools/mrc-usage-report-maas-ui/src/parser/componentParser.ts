...
export class ComponentParser {
	...
	/**
	 * Detects if a component has custom styling or overrides
	...
	private detectCustomization(path: any): ComponentUsage["customization"] {
		...
		while (parent) {
			if (
				...
				parent.node.init.callee.object &&
		...
		path.node.attributes.forEach((attr: any) => {
			if (t.isJSXAttribute(attr) && t.isJSXIdentifier(attr.name) && (attr.name.name === "style" || attr.name.name === "sx" || attr.name.name === "css")) {
				customization.customStyles = true;

				// Try to extract overridden properties
				if (attr.value && t.isJSXExpressionContainer(attr.value) && t.isObjectExpression(attr.value.expression)) {
