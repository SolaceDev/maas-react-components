    ...
    scanForFiles() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const mfe of this.mfes) {
                let mfePath;
                if (mfe === "maas-ops-ui") {
                    mfePath = this.basePath;
                }
                else {
                    mfePath = path_1.default.join(this.basePath, "micro-frontends", mfe);
                }
        ...
        return __awaiter(this, void 0, void 0, function* () {
                    ...
                    for (const file of files) {
                        ...
                        if (file.endsWith("index.ts") || file.endsWith("index.tsx")) {
                            ...
                            if (match && match[1] && match[2]) {
                                ...
                                if (!finalComponentPath.endsWith(".ts") && !finalComponentPath.endsWith(".tsx")) {
                                ...
                                allComponents.push({ name: componentName, path: finalComponentPath });
                            }
                        }
                    }
            ...
            if (actualMrcPath) {
                ...
                try {
                    ...
                    const { stdout } = yield execPromise(`find ${componentsPath} -type f \\( -name "*.ts" -o -name "*.tsx" -o -name "*.js" -o -name "*.jsx" \\) -not -name "*.d.ts" -not -name "*.test.*" -not -name "*.spec.*"`);
                    const componentFiles = stdout.trim().split("\n").filter(Boolean);
                    for (const file of componentFiles) {
                        const fileName = path_1.default.basename(file);
                        const componentName = path_1.default.parse(fileName).name;
                        // Exclude components with "Props", "Utils", "Icon", "use" in their name,
                        // React hooks (starting with "use"), and files in table/components subdirectory
                        const shouldExclude = componentName.includes("Props") ||
                            ...
                            file.includes("/table/components/");
                        if (!shouldExclude) {
