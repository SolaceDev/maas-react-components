...
import { MfeInfo, MrcSourceType } from "../types";
...
export class FileScanner {
	...
	async scanForFiles(): Promise<string[]> {
		...
		for (const mfeInfo of this.mfeInfos) {
			const mfePath = mfeInfo.path;
			...
			try {
				...
				const { stdout } = await execPromise(
					`find ${srcPath} -type f \\( -name "*.ts" -o -name "*.tsx" -o -name "*.js" -o -name "*.jsx" \\) -not -path "*/node_modules/*" -not -name "*.d.ts" -not -name "*.test.*" -not -name "*.spec.*"`
				);
	...
	async scanForMrcComponents(mrcPath: string): Promise<string[]> {
		...
		if (actualMrcPath) {
			...
			try {
				...
				const { stdout } = await execPromise(
					`find ${componentsPath} -type f \\( -name "*.ts" -o -name "*.tsx" \\) -not -name "*.d.ts" -not -name "*.test.*" -not -name "*.spec.*"`
				);
				...
				console.log(
					...
					} components (Props, Utils/utils, Icon, hooks, table/components)`
				);
	...
	private async getExportMappings(mrcPath: string): Promise<Map<string, string>> {
		...
		try {
			...
			console.log(
				`Found ${mappings.size} export mappings in index.tsx (excluded ${excludedCount} Props/Utils/Icon/hooks)`
			);
