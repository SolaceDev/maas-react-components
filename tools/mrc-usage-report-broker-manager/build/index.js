...
program
    ...
    .option("-r, --mrc-path <path>", "Path to the MRC repository", "/Users/ishanphadte/Desktop/maas-react-components")
...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            console.log(`  MFEs: ${mfes.length > 0 ? mfes.join(", ") : "broker-manager"}`);
            ...
            for (let i = 0; i < files.length; i++) {
                const file = files[i];
                const mfe = mfes.length > 0
                    ? file.split(path_1.default.sep).find((part) => mfes.includes(part)) || ""
                    : "broker-manager";
                try {
            ...
            if (mfes.length > 0) {
                ...
                try {
                    const packageJsonPath = path_1.default.join(basePath, "package.json");
                    if (fs_1.default.existsSync(packageJsonPath)) {
                        const packageJson = JSON.parse(fs_1.default.readFileSync(packageJsonPath, "utf8"));
                        const dependencies = packageJson.dependencies || {};
                        const devDependencies = packageJson.devDependencies || {};
            ...
            for (const format of outputFormats) {
                const outputPath = path_1.default.join(outputDir, `mrc-broker-manager-usage-report.${format}`);
                if (format === "html") {
                    console.log(chalk_1.default.blue("Generating HTML report..."));
                    const htmlReporter = new htmlReporter_1.HtmlReporter();
                    yield htmlReporter.generateReport(reportData, outputPath);
                    console.log(chalk_1.default.green("HTML report generation complete."));
                }
                else if (format === "json") {
                    console.log(chalk_1.default.blue("Generating JSON report..."));
                    const jsonOutput = JSON.stringify(reportData, null, 2);
                    fs_1.default.writeFileSync(outputPath, jsonOutput);
                    console.log(chalk_1.default.green(`JSON report generated at ${outputPath}`));
                }
                else if (format === "csv") {
