...
export class FileScanner {
	...
	async scanForFiles(): Promise<string[]> {
		...
		for (const mfe of this.mfes) {
			...
			if (mfe === "maas-ops-ui") {
				...
				mfePath = path.join(this.basePath, "micro-frontends", mfe);
			}
	 ...
	 */
	async scanForMrcComponents(mrcPath: string): Promise<{ name: string; path: string }[]> {
		...
		if (actualMrcPath) {
			...
			try {
				...
				const { stdout } = await execPromise(
					`find ${componentsPath} -type f \\( -name "*.ts" -o -name "*.tsx" -o -name "*.js" -o -name "*.jsx" \\) -not -name "*.d.ts" -not -name "*.test.*" -not -name "*.spec.*"`
				);
