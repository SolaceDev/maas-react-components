...
The SolaceTable component provides a comprehensive data table solution for displaying and interacting with structured data, supporting features like sorting, row selection, column hiding, expandable rows, pagination integration, and cross-page selection. This flexible table pattern presents structured data in an organized, interactive format designed to handle complex data scenarios while maintaining optimal performance and accessibility, helping users efficiently browse, analyze, and interact with large datasets through consistent visual hierarchy and intuitive interaction patterns.
...
- `theme.palette.primary.text.wMain` - Primary text color
- `theme.palette.secondary.w20` - Border colors for rows and columns
- `theme.palette.secondary.w40` - Secondary border colors
- `theme.palette.background.w10` - Table background color
- `theme.palette.accent.n2.wMain` - Focus and selection colors
