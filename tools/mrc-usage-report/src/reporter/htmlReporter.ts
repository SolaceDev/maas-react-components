...
<body>
  <div class="container">
    ...
    <div class="footer">
      <p>
        This report was generated by the MRC Usage Report tool.
      </p>
    </div>
  </div>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        document.querySelectorAll('.component-details').forEach(detail => {
          ...
          if (propNameFilter || propValueFilter) {
            ...
            instances.forEach(instance => {
              ...
              props.forEach(prop => {
                const name = prop.children[0].textContent.toLowerCase();
                const value = prop.children[1].textContent.toLowerCase();
                
                if ((!propNameFilter || name.includes(propNameFilter)) && 
                    (!propValueFilter || value.includes(propValueFilter))) {
                  hasPropMatch = true;
                }
              });
            });
          ...
          if (componentName.includes(componentNameFilter) && hasPropMatch) {
      ...
      // D3 Charts
      const mostUsedComponents = ${JSON.stringify(overallStats.mostUsedComponents)};
      const mfeUsages = ${JSON.stringify(
				Object.entries(overallStats.mfeUsages).map(([name, count]) => ({ name, count }))
			)};
