...
import { transformResponse } from "../utils/keyTransformer.js";
export async function getComponentUsageByMfe(applicationName, mfeName, componentName, propIdentifier) {
    // eslint-disable-next-line no-console
    console.error(`[DEBUG] Starting getComponentUsageByMfe for ${componentName} in MFE ${mfeName} of application ${applicationName}`);
    ...
    try {
        ...
        const instancesFile = response.data;
        if (!instancesFile) {
            // eslint-disable-next-line no-console
        ...
        if (axios.isAxiosError(error) && error.response?.status === 404) {
            // eslint-disable-next-line no-console
            console.error(`[INFO] instances.json not found for ${componentName} in ${mfeName} (This is expected if the component is not used).`);
            return [];
        }
