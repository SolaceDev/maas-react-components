  ...
  aggregate(
    ...
    for (const [
      ...
      const files = Array.from(
        new Set(componentUsages.map((usage) => usage.filePath))
      );
    ...
    // Most used props
    const allPropCounts = new Map<string, number>();
    for (const usage of usages) {
      for (const prop of usage.props) {
    ...
    const usedComponentNames = new Set(
      componentStats.map((s) => s.componentName)
    );
    const unusedComponents = allComponents.filter(
      (comp) => !usedComponentNames.has(comp.name)
    );
    ...
    for (const component of allComponents) {
      const stat = componentStats.find(
