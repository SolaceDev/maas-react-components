...
- Tooltips are text labels that appear on hover to provide supplemental information about an element. They help inform users and identify elements.
- Standard tooltips appear 500ms after user hover, fade in within 150ms, and fade out 200ms after the cursor leaves the trigger element.
- Rich tooltips are used for more detailed information, such as explaining functionalities via help bubbles or providing additional context about objects.
- Position tooltips carefully to ensure they do not obstruct users from performing other tasks on the screen.
